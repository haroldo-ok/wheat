...
function preProcessMarkdown(markdown) {
  ...
  // See if some custom processing is required.
  if (Options.preProcessMarkdown) {
	Options.preProcessMarkdown(props);
  }
...
function activateSnippets(version, snippets, canExecute, callback) {
  Step (
    ...
    function (err, files) {
      if (err) { 
		logger.warn("Data.activateSnippets: {}", err); 
		callback(err);
		return; 
      }
...
var Data = module.exports = {
  ...
  article: Git.safe(function article(version, name, callback) {
    ...
    Step(
      ...
      function finish(err, author) {
        if (err) { 
 		  logger.warn("Data.article.finish: {}", err); 
		  callback(err);
		  return; 
        }
  ...
  fullArticle: Git.safe(function fullArticle(version, name, callback) {
    ...
    Step(
      ...
      function loadExtras(err, props) {
        if (err) { 
		  logger.warn("Data.fullArticle.loadExtras: {}", err); 
		  callback(err);
		  return; 
        }
        article = props;
      ...
      function finish(err, articles, log, snippets) {
        if (err) { 
		  logger.warn("Data.fullArticle.finish: {}", err); 
		  callback(err);
		  return; 
        }
  ...
  author: Git.safe(function author(version, name, callback) {
    Step(
      ...
      function process(err, markdown) {
        if (err) { 
		  logger.warn("Data.author.process: {}", err); 
		  callback(err);
		  return; 
        }
      ...
      function finish(err, props) {
        if (err) { 
		  logger.warn("Data.author.finish: {}", err); 
		  callback(err);
		  return; 
        }
  ...
  categories: Git.safe(function articles(version, callback) {
    Step(
      ...
      function readArticles(err, results) {
        if (err) { 
		  logger.warn("Data.categories.readArticles: {}", err); 
		  callback(err);
		  return; 
        }
      ...
      function processCategories(err, articles) {
        if (err) { 
		  logger.warn("Data.categories.processCategories: {}", err); 
		  callback(err);
		  return; 
        }
  ...
  fullArticles: Git.safe(function fullArticles(version, callback) {
    Step(
      ...
      function readArticles(err, results) {
        if (err) { 
		  logger.warn("Data.fullArticles.readArticles: {}", err); 
		  callback(err);
		  return; 
        }
      ...
      function sortAndFinish(err, articles) {
        if (err) { 
		  logger.warn("Data.fullArticles.sortAndFinish: {}", err); 
		  callback(err);
		  return; 
        }
  ...
  articles: Git.safe(function articles(version, callback) {
    Step(
      ...
      function readArticles(err, results) {
        if (err) { 
		  logger.warn("Data.articles.readArticles: {}", err); 
		  callback(err);
		  return; 
        }
      ...
      function sortAndFinish(err, articles) {
        if (err) { 
		  logger.warn("Data.articles.sortAndFinish: {}", err); 
		  callback(err);
		  return; 
        }
